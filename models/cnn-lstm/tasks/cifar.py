...
class CifarTask:
    def __init__(self, device, timer, architecture, seed, reducer):
        self._device = device
        self._timer = timer
        self._seed = seed
        ...
        if reducer == 'VarianceReducer':
            self._model = ModuleValidator.fix(self._model)
            self._model = GradSampleModuleExpandedWeights(self._model)
    ...
    def batch_loss_and_gradient(
        ...
        for parameter in self._model.parameters():
            if parameter.grad is None:
                parameter.grad= parameter.grad_sample.mean(axis=0).detach()
            if hasattr(parameter, 'grad_sample'):
                parameter.grad.data.copy_( parameter.grad_sample.mean(axis=0))
        # for parameter in self._model.parameters():
        #     print(parameter.grad_sample.mean(axis=0)[0])  # Inspect per-sample gradients
        #     print('-----***')
        #     print(parameter.grad[0]) 
            # break
