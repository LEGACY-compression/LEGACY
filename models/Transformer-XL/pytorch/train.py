...
def parse_args():
    ...
    general.add_argument('--method', type=str, default="Topk-layer")
    general.add_argument('--extras', type=str, default="00")
    general.add_argument('--memory', type=int, default=0)
    # general.add_argument('--epochs', type=int, default=200)  #use max_step instead
    general.add_argument('--size_thres', type=int, default=10000)
    general.add_argument('--easy_compress', type=float, default=30)
    general.add_argument('--agg_compress', type=float, default=10)
    general.add_argument('--log_dir', type=str, default='./all_logs/final')

    general.add_argument('--group_splits', type=str, default='', help='threshold to create groups per layer size')
    general.add_argument('--group_compressions', type=str, default='1,0.1', help='compression ratio % to use per group')
    ...
    val.add_argument('--eval_max_steps', type=int, default=-1,
                     help='Max eval steps')
    val.add_argument('--eval_interval', type=int, default=150,
...
def get_compression_ratio2():
    return(sum(communication)-communication[0]-communication[1])/(len(communication)-2)
def get_communicated_layers():
    return communicated_layers

communicated_layers=[]
def layerwise_compressed_comm(
    model, world_size, method=None, current_epoch="", 
):
    ...
    for index, layer in enumerate(model.parameters()):
        ...
        if method =='Randomk':
            ...
            compress_grad = adacomp(flatten_grad, index)
        ...
        if compress_grad.numel() > 0:
            ...
            flatten_grad.zero_()
    communication.append(iter_volume/total_size)



# def layerwise_compressed_comm(
#     model,method=None, extras=""
# ):
#     model_size = len(list(model.parameters()))
#     communication[1]+=1
#     communicated.append([])
...
#             compress_grad = randomk_layer(flatten_grad, extras)
#         elif method == "Randomk-level":
#             compress_grad = randomk_level(flatten_grad, extras, index, model_size)
#         elif method == "Randomk-adaptative" and extras:
#             compress_grad = randomk_adaptative(flatten_grad, extras)

#         elif method == 'Topk-level':
#             compress_grad = topk_level(flatten_grad, extras, index, model_size)
#         elif method == 'Topk-layer':
#             compress_grad = topk_layer(flatten_grad, extras)
#         elif method == "Topk-adaptative":
#             compress_grad = topk_adaptative(flatten_grad, extras)
#         elif method == "Thresholdv":
#             # Threshold V
#             V=float(extras[1])
#             # print(f'using thers {V}')
#             flatten_grad_abs = flatten_grad.abs()
#             flatten_grad[flatten_grad_abs < V] = 0
#             compress_grad = flatten_grad.clone()
            
#             # iter_volume+=torch.count_nonzero(compress_grad).item()

#         else:
#             raise IOError(str(method)+ " compressor method not found")
#         non_zero_grad=torch.count_nonzero(compress_grad).item()
#         # print(compress_grad.numel(), non_zero_grad)
#         communicated[-1].append(non_zero_grad)
#         communication[0]= communication[0]+non_zero_grad
...
def train(tr_iter, va_iter, model, para_model, mems, model_config, optimizer,
    ...
    global communication,communicated, ppl_val,throughputs_loginter, time_loginter
    ...
    for batch, (data, target, seq_len, _) in enumerate(train_iter, start=last_batch+1):
        ...
        layerwise_compressed_comm(
                    model,args.world_size, args.method, train_step,
                )
        ...
        if (do_periodic_eval or is_final_step or interrupted) and not args.no_eval:
            ...
            ppl_val.append( (train_step,  math.exp(val_loss)))
            per_eval_elapsed=time.time()-per_eval_time
            per_eval_time=time.time()
            per_eval_elapsed=utils.distributed.all_reduce_item(per_eval_elapsed, op='max')
            per_eval_throughput = per_eval_tokens / per_eval_elapsed
            per_eval_throughput= utils.distributed.all_reduce_item(per_eval_throughput, op='sum')
            throughputs_loginter.append(per_eval_throughput)
            per_eval_tokens = 0
            time_loginter.append(per_eval_elapsed)
            ...
            if args.local_rank==0:
                print(f'throuput {throughputs_loginter[-1]} , time {time_loginter[-1]}, transmited in last step {sum(communicated_layers[-1])}')
...
ppl_val=[]
throughputs_loginter=[]
time_loginter = []
def main():
    args = parse_args()

    group_splits, group_compressions, epochs = args.group_splits, args.group_compressions, args.max_step
    ...
    args.world_size = utils.distributed.get_world_size()
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            model_layers.append(layer_params)
    ...
    # print('first',sum(l[:len(l)//3]),'last',sum(l[len(l)//3:]))
    # print(l)
    scaler = None
    if args.fp16:
        if args.amp == 'pytorch':
    ...
    if args.local_rank==0:
        compression_ratio=round(get_compression_ratio(total_params)*100,3)
        print("The compression ratio is :",str(round(compression_ratio,2)) )
        print('valid ppl', ppl_val[-1])


        compression_ratio=get_compression_ratio(total_params)*100
        d={"version": '1.0', "test_accuracy":ppl_val, "layer_transmit":communicated_layers, 'throughput_per_epouch': throughputs_loginter,
           'time_per_epoch': time_loginter, 'total_time': time.time() - start_time, 'model_layers': model_layers, 'total_params': sum(model_layers),
           "compression_method": args.method, 'epochs': epochs, "global_compression_ratio": compression_ratio, 'network':'Transformer-XL',
            "group_compressions": args.group_compressions, "group_splits": args.group_splits,
             "n_worker": utils.distributed.get_world_size(), 'memory': False}
        
        compression_param = f"-split-thersh({args.group_splits}),group-ratio({args.group_compressions})"
        results_file='_'.join(['test'+str(args.seed), 'wt103', 'TrXL', str(args.method)+compression_param, 'steps-'+str(epochs),
                                str(compression_ratio)+'-ratio', str(ppl_val[-1])+'-acc',str(utils.distributed.get_world_size())+ 'nodes'])
        results_file+='logs'
        with open(os.path.join(os.path.expanduser(args.log_dir), results_file), 'w') as json_file:
                    json.dump(d, json_file)
        # with open(os.path.join(os.path.expanduser(args.log_dir), results_file), 'a') as f:
        #     f.write("compression rate is :"+str(round(compression_ratio,2))+'\n')
        #     f.write("total training time is :"+str(round(time.time()-start_time,2))+"s\n")
        #     f.write("step\tvalid_ppl\n")
        #     for ep in range(len(ppl_val)):
        #         f.write(str(ppl_val[ep][0])+"\t"+str(round(ppl_val[ep][1], 2))+"\n")
    if not passed:
        sys.exit(1)
