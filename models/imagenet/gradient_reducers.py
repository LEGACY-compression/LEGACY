...
class TopKLayerSizeReducer(Reducer):
    ...
    """
    def __init__(self, random_seed, device, group_splits='1000', group_compressions='1,0.1'):
...
class ExactReducer(Reducer):
    def reduce(self, grad_in, grad_out):
        ...
        bits_communicated, params_transmitted = reduce_mean_list(self.device, list_in, list_out)

        return bits_communicated, params_transmitted


def reduce_mean_list(
    device: torch.device, list_in: List[torch.Tensor], list_out: List[torch.Tensor]
):
    if torch.distributed.is_available():
